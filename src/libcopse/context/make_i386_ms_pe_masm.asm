...
cps_context_new_from_sp PROC EXPORT
    mov  eax,         [esp+04h]     ; load 1. arg of cps_context_new_from_sp, pointer to context stack (base)
    lea  eax,         [eax-038h]    ; reserve space for cps_context at top of context stack

    ; shift address in EAX to lower 16 byte boundary
    ; == pointer to cps_context and address of context stack
    and  eax,         -16

    mov  ecx,         [esp+04h]     ; load 1. arg of cps_context_new_from_sp, pointer to context stack (base)
    mov  [eax+018h],  ecx           ; save address of context stack (base) in cps_context
    mov  edx,         [esp+08h]     ; load 2. arg of cps_context_new_from_sp, context stack size
    mov  [eax+01ch],  edx           ; save context stack size in cps_context
    neg  edx                        ; negate stack size for LEA instruction (== substraction)
    lea  ecx,         [ecx+edx]     ; compute bottom address of context stack (limit)
    mov  [eax+020h],  ecx           ; save address of context stack (limit) in cps_context
    mov  [eax+034h],  ecx           ; save address of context stack limit as 'dealloction stack'
    mov  ecx,         [esp+0ch]     ; load 3. arg of cps_context_new_from_sp, pointer to context function
    mov  [eax+014h],  ecx           ; save address of context function in cps_context
